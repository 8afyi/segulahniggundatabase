...
usage() {
  cat <<'USAGE'
Usage:
  sudo ./scripts/deploy-production-debian13.sh \
...
Notes:
- Run as root on a Debian 13 host.
...
require_debian13() {
  ...
  if [[ "${ID:-}" != "debian" ]]; then
    fail "Unsupported OS (${ID:-unknown}). Use this script only on Debian 13."
  fi

  local major_version="${VERSION_ID%%.*}"
  if [[ "$major_version" != "13" ]]; then
    fail "Unsupported Debian version (${VERSION_ID:-unknown}). This script targets Debian 13."
  fi
}
